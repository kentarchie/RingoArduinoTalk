...
Ringo Robot:  RingoHardware  Rev02.02  08/2015
...
void TxIR(unsigned char *Data, int Length){//Ver. 1.0, Dustin Soodak
    ...
    delayMicroseconds(8900); //was 9000
    ModulateIR(0,0);
    delayMicroseconds(4500);
    ModulateIR(Freq,UsOn);
    delayMicroseconds(550);  //was 600
    for(i=0;i<Length;i++){      
      for(j=0;j<8;j++){
        ModulateIR(0,0);       //ModulateIR(0);
        if((Data[i])&(1<<j))
          delayMicroseconds(1550);  //was 1600
        else
          delayMicroseconds(475); //was 525
        ModulateIR(Freq,UsOn);
        delayMicroseconds(500);  //was 600
      }      
    }//end for(i=0;i<Length;i++)    
    ModulateIR(0,0);  //ModulateIR(38000, 0);
}//end TxIR()
